...
export function UserCreateForm({ currentRole, availableCountries }: UserCreateFormProps) {
  const router = useRouter();
  const roleOptions = currentRole === "SUPER_ADMIN" ? ALL_ROLE_OPTIONS : NON_ADMIN_ROLE_OPTIONS;
  const defaultRole = roleOptions[0] ?? "EDITOR";
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    if (!draft.name.trim() || !draft.email.trim() || !draft.password) {
      setStatus("Completa nombre, email y contraseña.");
      return;
    }
  ...
  return (
    <section style={cardStyle}>
      <div style={{ display: "flex", flexDirection: "column", gap: "var(--space-3)" }}>
        <div>
          <h2 style={{ margin: 0, fontSize: "1rem" }}>Nuevo usuario</h2>
          <p style={{ margin: "4px 0 0", color: "var(--color-neutral-600)" }}>
            {currentRole === "SUPER_ADMIN"
              ? "Puede crear cualquier rol y administrar países libremente."
              : "Solo puedes crear EDITOR o ASSISTANT dentro de tus países asignados."}
          </p>
        </div>
